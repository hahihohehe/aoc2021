...
main = do
  ...
  putStrLn $ show $ findLastWin num boards
  putStrLn $ show $ (score $ snd $ findLastWin num boards) * (fst $ findLastWin num boards)
...
findLastWin :: [Int] -> [Board] -> (Int, Board)
findLastWin (x:xs) boards
  | (length $ findWins $ performRound x boards) > 0 && length boards == 1 = (x, head $ findWins $ performRound x boards)
  | otherwise = findLastWin xs $ performRound x (filter (not . checkWin) boards)
